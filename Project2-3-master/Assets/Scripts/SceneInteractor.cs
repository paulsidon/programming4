            ...
            public class SceneInteractor : MonoBehaviour
            {
                ...
                void SetLightingIntensity(GameObject go)
                {
                    ...
                    **/
                    ...
                    float intensityValue = go.GetComponent<LightingIntensityValue>().intensity;
                    //Debug.Log(intensityValue);
                    GameObject[] allDynamicLights = GameObject.FindGameObjectsWithTag("DynamicLights");
                    foreach (GameObject lightObject in allDynamicLights)
                        lightObject.GetComponent<Light>().intensity = intensityValue;
                ...
                void SetLightingColor(Color c)
                {
                   ...
                   **/
                    GameObject[] allDynamicLights = GameObject.FindGameObjectsWithTag("DynamicLights");
                    foreach (GameObject go in allDynamicLights)
                ...
                void OnCollisionEnter(Collision c)
                {
                    ...
                    if (gameObjectThatWasHit != "Floor1" || gameObjectThatWasHit != "Floor2" || gameObjectThatWasHit != "TerrainGroup_0")
                    {
                        source.Play();
                    }
                    /**
                    ...
                    **/
                    ...
                    if (gameObjectThatWasHit.Equals("RedCube"))
                        SetLightingColor(Color.red);

                    if (gameObjectThatWasHit.Equals("GreenCube"))
                        SetLightingColor(Color.green);

                    if (gameObjectThatWasHit.Equals("BlueCube"))
                        SetLightingColor(Color.blue);

                    if (gameObjectThatWasHit.Equals("RugCarpet"))
                        SetLightingColor(Color.white);

                    if (gameObjectThatWasHit.Contains("IntensityCube"))
                        SetLightingIntensity(c.gameObject);

                    if (gameObjectThatWasHit.Equals("DoorRotationAxis"))
                    {
